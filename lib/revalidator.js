(function (exports) {
  ...
  function validate(object, schema, options) {
    ...
    validateObject(object, schema, options, null, errors);
  ...
  function validateObject(object, schema, options, arrayIndex, errors) {
    ...
    if (schema.properties) {
      ...
      for (var p in props) {
        if (props.hasOwnProperty(p)) {
          visitedProps.push(p);
          validateProperty(object, object[p], p, props[p], options, arrayIndex, errors);
        }
      }
    }
        ...
        if (props.hasOwnProperty(p)) {
          ...
          for (var k in object) {
            if (object.hasOwnProperty(k)) {
              visitedProps.push(k);
              if (re.exec(k) !== null) {
                validateProperty(object, object[k], p, props[p], options, arrayIndex, errors);
              }
            }
          }
        }
    ...
    if (undefined !== schema.additionalProperties) {
      ...
      if (schema.additionalProperties === false && unvisitedProps.length > 0) {
        for (i = 0, l = unvisitedProps.length; i < l; i++) {
          error("additionalProperties", unvisitedProps[i], object[unvisitedProps[i]], schema, arrayIndex, errors);
        }
      }
      // additionalProperties is a schema and validate unvisited properties against that schema
        ...
        for (i = 0, l = unvisitedProps.length; i < l; i++) {
          validateProperty(object, object[unvisitedProps[i]], unvisitedProps[i], schema.additionalProperties, options, arrayIndex, errors);
        }
      }
    }
  ...
  function validateProperty(object, value, property, schema, options, arrayIndex, errors) {
    ...
    function constrain(name, value, assert) {
      if (schema[name] !== undefined && !assert(value, schema[name])) {
        error(name, property, value, schema, arrayIndex, errors);
      }
    }

    if (value === undefined) {
      if (schema.required && schema.type !== 'any') {
        return error('required', property, undefined, schema, arrayIndex, errors);
      } else {
        return;
      }
    }
    ...
    if (typeof schema.dependencies === 'string' &&
      ...
      error('dependencies', property, null, schema, arrayIndex, errors);
    }
    ...
    if (typeof schema.dependencies === 'object') {
      validateObject(object, schema.dependencies, options, arrayIndex, errors);
    }

    checkType(value, schema.type, function(err, type) {
      if (err) return error('type', property, typeof value, schema, arrayIndex, errors);
      ...
      if (schema.format && options.validateFormats) {
        ...
        if (!spec) {
          if (options.validateFormatsStrict) {
            return error('format', property, value, schema, arrayIndex, errors);
          }
        }
        else {
          if (type !== 'null' && !spec.test(value)) {
            return error('format', property, value, schema, arrayIndex, errors);
          }
        }
      }
      ...
      if (schema['enum'] && schema['enum'].indexOf(value) === -1) {
        error('enum', property, value, schema, arrayIndex, errors);
      }
      ...
      switch (type || (isArray(value) ? 'array' : typeof value)) {
        ...
        case 'array':
          constrain('items', value, function (a, e) {
            for (var i = 0, l = a.length; i < l; i++) {
              validateProperty(object, a[i], property, e, options, i, errors);
            }
            return true;
          });
        ...
        case 'object':
          ...
          if (schema.properties || schema.patternProperties || schema.additionalProperties) {
            validateObject(value, schema, options, arrayIndex, errors);
          }
          break;
      }
    });
  ...
  function checkType(val, type, callback) {
    ...
    for (var i = 0, l = types.length; i < l; i++) {
      ...
      if (type === 'string' ? isString(val) :
          ...
          type === 'object' ? isObject(val) :
          type === 'number' ? isNumber(val) :
          type === 'integer' ? isInteger(val) :
          type === 'null' ? isNull(val) :
          type === 'boolean'? isBoolean(val) :
          type === 'any' ? isAny(val) : false) {
  ...
  function error(attribute, property, actual, schema, arrayIndex, errors) {
    if (arrayIndex !== undefined && arrayIndex !== null) {
        arrayIndex = arrayIndex.toString();
    } else {
        arrayIndex = 'null';
    }
    var lookup = { expected: schema[attribute], attribute: attribute, property: property, index: arrayIndex };
